         ...
         <table border="0" cellpadding="0" cellspacing="0" width="900px">
			<tr align="center" valign="middle">
				<td>
					...
					<c:forEach var="i" begin="${startPage}" end="${endPage}"> <!-- 페이지 번호는 반복을 돌면서 열이 늘어나는 것이므로 tr안에 td를 반복하기 위해 forEach를 넣어준다. -->
						<c:choose>
							...
							<c:otherwise>
								<!-- 페이지를 아주는것이다. -->
								<a href="${pageContext.request.contextPath}/board/BoardList.bo?page=${i}">${i}%nbsp;</a>
							</c:otherwise>
						</c:choose>								
					</c:forEach>
